...
# file paths for summary files per injection case
path_6310_488 <- here("derived_data", "data_I6310_488.csv")
path_6310_555 <- here("derived_data", "data_I6310_555.csv")
path_6313_555 <- here("derived_data", "data_I6313_555.csv")
...
# Extractors for identifier patterns from strings
animal_ptn <- "I[0-9]{4}"
...
# join section data per case and save the results
...
write_csv(df_6310_488, path_6310_488)
write_csv(df_6310_555, path_6310_555)
write_csv(df_6313_555, path_6313_555)

df_6310_488_summary <- df_6310_488 %>% 
  rename(name = Name, `488` = CTB.Alexa.488) %>% 
  mutate(area = `Area.(µm²)` / 1e6, side = ifelse(str_detect(name, "_c"), "contra", "ipsi")) %>% 
  group_by(name, side) %>% 
  summarise(counts = sum(`488`), sections = n(), density = (sum(`488`))/(sum(area))) %>% 
  group_by(side) %>% mutate(label = 100 * counts/max(counts)) %>% ungroup() 

df_6310_555_summary <- df_6310_555 %>% 
  rename(name = Name, `555` = Alexa.555) %>% 
  mutate(area = `Area.(µm²)` / 1e6, side = ifelse(str_detect(name, "_c"), "contra", "ipsi")) %>% 
  group_by(name, side) %>% 
  summarise(counts = sum(`555`), sections = n(), density = (sum(`555`))/(sum(area))) %>% 
  group_by(side) %>% mutate(label = 100 * counts/max(counts)) %>% ungroup()

df_6313_555_summary <- df_6313_555 %>% 
  rename(name = Name, `555` = Alexa.555) %>%  
  mutate(area = `Area.(µm²)` / 1e6, side = ifelse(str_detect(name, "_c"), "contra", "ipsi")) %>% 
  group_by(name, side) %>% 
  summarise(counts = sum(`555`), sections = n(), density = (sum(`555`))/(sum(area))) %>% 
...
df_summary_counts_all <- df_summary_counts %>% 
  mutate(name = str_replace(name, "_c", ""))
...
g_raster <- ggplot(df_summary_label_tall, 
  ...
  scale_fill_viridis_c() + labs(y = NULL, fill = "cell count %", 
                                ...
                                title = "Manual cell counts in marmoset cortex") + 
